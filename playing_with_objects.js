...
function guess() {
  ...
  const thisGuess = "guess" + rowCount;
  guesses[thisGuess] = [];
  console.log(guesses);
  ...
  buttons.forEach( button => {
    button.addEventListener("click", (e) => {
      ...
      if(letter == "DEL") {
        guesses[thisGuess].pop();
      } else if (letter == "ENTER"){
        const myWord = guesses[thisGuess].join("");
        finalGuess(myWord);
      } else {
        if (guesses[thisGuess].length < wordle.length) {
          guesses[thisGuess].push(letter);
        }
      }

      for(let i = 0; i < boxes.length; i++){
        if (guesses[thisGuess[i]]) {
          boxes[i].innerHTML = `<h3>${guesses.thisGuess[i]}`;
        } else {
          boxes[i].innerHTML = "";
        }
      }
